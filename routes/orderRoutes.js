...
router.post("/", upload.array("files"), async (req, res) => {
  try {
    ...
    for (let i = 0; i < req.files.length; i++) {
      ...
      // 1️⃣ Handle Page Range
      let pagesToPrint = meta.pageCount;

      if (meta.pageRange && meta.pageRange !== "all") {
      ...
      // 2️⃣ Handle Double Side
      let sheetsToPrint = pagesToPrint;

      if (meta.printSide === "double") {
        sheetsToPrint = Math.ceil(pagesToPrint / 2);
      }

      // 3️⃣ Determine Price Per Sheet
      let pricePerSheet = 0;

      if (meta.paperSize === "A4") {
        pricePerSheet = meta.color === "bw" ? 2 : 5;
      } else if (meta.paperSize === "A3") {
        pricePerSheet = meta.color === "bw" ? 4 : 8;
      }

      // 4️⃣ Final Price
      const calculatedPrice = sheetsToPrint * pricePerSheet * meta.copies;
